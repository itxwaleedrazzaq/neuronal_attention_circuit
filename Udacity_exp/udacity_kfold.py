...
def build_model(cell_type, input_shape=INPUT_SHAPE, num_classes=1):
    ...
    if cell_type == "RNNCell":
        ...
        x = NAC(d_model=64, num_heads=16, mode='euler',dt=1.0,sparsity=0.7, topk=10, euler_steps=6, return_sequences=False)(x)
        ...
        x = NAC_FC(d_model=64, num_heads=16, mode="exact",delta_t=1.0,sparsity=0.7, topk=10, return_sequences=False)(x)
    elif cell_type == "NAC_PW":
        x = NAC_PW(d_model=64, num_heads=16, mode="exact",delta_t=1.0,sparsity=0.7, return_sequences=False)(x)
