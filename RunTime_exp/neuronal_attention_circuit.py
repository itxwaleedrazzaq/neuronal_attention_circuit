...
class NAC(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # RNNs that compute phi and tau
        self.phi_rnn = self._make_inter_to_motor_projections("phi")
        self.tau_rnn = self._make_inter_to_motor_projections("tau")
    ...
    def compute_phi_tau(self, q, k, t):
        ...
        phi_raw = self.phi_rnn(flat_3d)
        phi = tf.reshape(tf.nn.sigmoid(phi_raw), [B, H, Tq, K_eff])

        tau_raw = self.tau_rnn(flat_3d)
