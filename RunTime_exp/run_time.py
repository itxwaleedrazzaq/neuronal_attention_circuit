...
from baseline_cells import CTRNNCell, ODELSTM, CTGRU, GRUD, PhasedLSTM, GRUODE, ODEformer,CTA
...
def build_model(cell_type, seq_len=sequence_length, hidden_dim=hidden_dim, num_heads=num_heads):
    ...
    if cell_type == "RNNCell":
        ...
        x = NAC(d_model=hidden_dim, num_heads=num_heads, mode="exact", return_sequences=False)(inp)
    elif cell_type == "NAC_Euler":
        x = NAC(d_model=hidden_dim, num_heads=num_heads, mode="euler", euler_steps=5, return_sequences=False)(inp)
    elif cell_type == "NAC_Steady":
        x = NAC(d_model=hidden_dim, num_heads=num_heads, mode="steady", euler_steps=5, return_sequences=False)(inp)
    elif cell_type == "NAC_FC":
        x = NAC_FC(d_model=hidden_dim, num_heads=num_heads, mode="exact", return_sequences=False)(inp)
    elif cell_type == "NAC_PW":
        x = NAC_PW(d_model=hidden_dim, num_heads=num_heads, mode="exact", return_sequences=False)(inp)
    elif cell_type == "NAC_PWFC":
        x = NAC_PWFC(d_model=hidden_dim, num_heads=num_heads, mode="exact", return_sequences=False)(inp)
    else:
